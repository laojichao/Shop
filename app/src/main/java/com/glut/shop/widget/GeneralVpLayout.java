...
import com.glut.shop.adapter.GeneralVpAdapter;
import com.glut.shop.widget.GeneralViewPager;
...
public class GeneralVpLayout<T> extends FrameLayout {
    ...
    private void init() {
        ...
        //构造指示器布局
        LayoutParams bottomParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    public GeneralVpLayout init(HolderCreator holderCreator, List<T> datas) {
        ...
        if (generalVpAdapter == null) {
            ...
            generalVpAdapter.reflashData(holderCreator, mDatas, isOpenInfiniteWheel);
        }
        //初始化数据
     ...
     * @return
     */
    public GeneralVpLayout setPageIndicator(int selectedRes, int defaultRes) {
     ...
     */
    public void start() {
        if (mImageViews == null || selectedRes == 0 || defaultRes == 0) {
     ...
     * 设置指示器是否可见
     *
     * @param isVisible true 可见 false 不可见 默认 true
     * @return
     */
    ...
    public GeneralVpLayout setOpenInfiniteWheel(boolean bool) {
        if (generalVpAdapter != null) {
            ...
            generalVpAdapter.reflashData(isOpenInfiniteWheel);
        }
        return this;
    }
     ...
     * 设置ViewPager是否可以手动滚动
     *
     * @param scrollble true 可以 false 不可以 默认 true
     * @return
     */
    ...
    //销毁
    public GeneralVpLayout destroy() {
        stop();
